        ...
        if not request.is_json:
            ...
            if not username or not password:
                ...
                if r:
                    ...
                    return jsonify({"msg": "Email หรือ Password ไม่ถูกต้อง"}), 401
    except Exception as e:
        print('error ===', e)
        current_app.logger.info(e)
        return jsonify(str(e))
# ----------------------------- Show Admin ---------------------------User Level 2&3 -c
@app.route('/api/v1/show_admin', methods=["POST"])
@connect_sql()
...
# ----------------------------- Delete Admin ---------------------------User Level 2&3 -c
@app.route('/api/v1/delete_admin', methods=["POST"])
@connect_sql()
def delete_admin(cursor):
    try:
        if not request.is_json:
            return jsonify({"msg": "Missing JSON in request"}), 400
        else:
            ...
            if not user_id or not role_user or not user_id_add:
                ...
                if role[0]['i'] == 1:
                    print("hh1")
                    return jsonify({"msg": "you cant access to this action"}), 401
                elif role[0]['i'] == 2:
                    if role_user == "3":
                        return jsonify({"msg": "you cant access to this action"}), 401
                    else:
                        ...
                        raw = json.loads(raw)
                        if 'message' in raw:
                            return jsonify({"msg": "dont have this user"}), 401
                        else:
                            sql = """SELECT role FROM user WHERE userid=%s"""
                            cursor.execute(sql, (user_id_add))
                            inrole = toJson(cursor.fetchall(), 'i')
...
#                 elif role[0]['i'] == 1:
#                     print("hh1")
#                     sql = """SELECT * FROM debt WHERE id_user = %s and status != 'สิ้นสุด' ORDER BY create_at DESC"""
#                     cursor.execute(sql, (user_id))
#                     columns = [column[0] for column in cursor.description]
#                     result = toJson(cursor.fetchall(), columns)
...
#                 elif role[0]['i'] == 2 or role[0]['i'] == 3:
#                     print("hhh2")
#                     sql = """SELECT * FROM debt WHERE status !='สิ้นสุด' ORDER BY create_at DESC"""
#                     cursor.execute(sql)
#                     columns = [column[0] for column in cursor.description]
#                     result = toJson(cursor.fetchall(), columns)
#                     for employee in result:
